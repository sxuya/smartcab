...
class LearningAgent(Agent):
    ...
    def createQ(self, state):
        ...
        # states_old = set([s for s, aQv in self.Q])
        # length_old = len(states_old)
    ...
    def choose_action(self, state):
        ...
        # WHAT'S MEAN OF THE SENTENCE OF "'epsilon' probability ", WHY IS IT STILL A random action
        #   Otherwise, choose an action with the highest Q-value for the current state
        if self.learning:
            if random.random() < self.epsilon:
                action = random.choice(self.valid_actions)
            else:
                # maxQ = self.get_maxQ(self.state) # only define the function, not do action
                maxQ = self.get_maxQ(state)
                # actions_maxQ = [a for a, v in self.Q[state].item() if v == maxQ]
                # @@ SO SAD, THE SPELLING IS WRONG, SO STUPID...
                actions_maxQ = [a for a, v in self.Q[state].items() if v == maxQ]
...
def run():
    ...
    # because the \epsilon value is minus when the time is bigger than 10
    # 2. set 'n_test' to 20, \epsilon = math.exp(-self.alpha * self.trialNum)
    ## result:FF
    # 3. add the 'n_test' to 40
    # no change, stll FF
    # 4. 
